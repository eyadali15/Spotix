/**
 * Spotix — Owner Profile (Wash + Parking, Light Mode)
 * Added: About, Contact, Policies links
 */
import React from 'react';
import { View, Text, StyleSheet, Alert, Pressable, Animated, ScrollView, Linking } from 'react-native';
  ...
  return (
    <>
      <Pressable onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); onPress?.(); }} style={styles.settingRow}>
        <View style={styles.settingLeft}><Text style={styles.settingIcon}>{icon}</Text><Text style={styles.settingLabel}>{label}</Text></View>
...
export default function OwnerProfileScreen() {
  ...
  const isWash = user?.role === 'WASH_OWNER';
  const roleIcon = isWash ? '🚿' : '🅿️';
  ...
  const showAbout = () => {
    Alert.alert('About Spotix', 'Spotix v1.0.0\n\nEgypt\'s premier smart parking and car care platform.\n\nBuilt with ❤️ in Egypt.\n\n© 2026 Spotix™');
  };
  const showContact = () => {
    Alert.alert('Contact Us', '📧 support@spotix.eg\n📞 +20 100 123 4567\n💬 WhatsApp: +20 100 123 4567\n📍 Cairo, Smart Village\n\nSat–Thu: 9AM–10PM\nFri: 12PM–8PM', [
      { text: 'Email', onPress: () => Linking.openURL('mailto:support@spotix.eg') },
      { text: 'Close', style: 'cancel' },
    ]);
  };
  const showPolicies = () => {
    Alert.alert('Terms & Policies', '📋 Terms of Service\n• Bookings subject to availability\n• Users must be 18+\n\n🔒 Privacy\n• Data encrypted and secure\n• We never sell your info\n\n💰 Cancellation\n• Free cancellation before check-in\n• No-shows may incur charges\n\n© 2026 Spotix™');
  };
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <Animated.View style={[styles.section, userAnim]}>
          <Card style={styles.userCard}>
            ...
            <View style={styles.roleBadge}><Text style={styles.roleEmoji}>{roleIcon}</Text><Text style={styles.roleText}>{t('owner')}</Text></View>
          </Card>
        </Animated.View>
        ...
        <Animated.View style={[styles.section, infoAnim]}>
          ...
          <Card style={styles.settingsCard}>
            <MenuItem icon="ℹ️" label="About Spotix" onPress={showAbout} />
            <MenuItem icon="📞" label="Contact Us" onPress={showContact} />
            <MenuItem icon="📋" label="Terms & Policies" onPress={showPolicies} />
          </Card>
        </Animated.View>
...
const styles = StyleSheet.create({
  ...
  roleBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: Colors.primaryFaded, paddingHorizontal: 14, paddingVertical: 6, borderRadius: Radius.full, gap: 6 },
  roleEmoji: { fontSize: 14 },
  roleText: { fontSize: FontSizes.small, fontWeight: FontWeights.semibold, color: Colors.primary },
